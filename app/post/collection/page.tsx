...
async function CollectionGrid({ session }: { session: Session | null }) {
  ...
  const collectionsArray = (Array.isArray(collections) ? collections : []).filter(
    (c: Collection) => (c.posts?.length ?? 0) > 0
  );
...
const CollectionCard = ({ collection }: { collection: Collection }) => {
  const imgUrl = collection.header_image?.url;
  const postCount = collection.posts?.length ?? 0;
  const isDraft = !collection.publishedAt;

  return (
    <Link
      href={`/post/collection/${collection.documentId}`}
    ...
    >
      <div className="relative w-full h-auto aspect-3/2">
        {imgUrl ? (
          ...
          <div
            ...
            aria-label={`Thumbnail for ${collection.title}`}
          />
        )}
        {isDraft && <DraftLabel />}
      </div>
      <div className="p-4 flex-1 flex flex-col">
        ...
        <p className="text-sm flex-1">{collection.summary}</p>
